...
- The scan ACA Job has no ingress, one replica per execution, 2 CPU / 4 GiB, a bounded timeout, and the ClamAV Azure Files share mounted through the read-only environment storage definition. The scheduled refresh job runs weekly at `10 19 * * 0` UTC against the same share through a separate read/write definition. It must stage and validate a complete `main`/`daily`/`bytecode` set, promote it to an immutable versioned directory, atomically replace the sanitized manifest last, and retain the previous active directory on every failure. Deployment must bootstrap and wait for one successful refresh before enabling the queue scanner. Signature age is warning-only: it is never an age-based publication block. Warn when the active signature set reaches 7 days; a valid last-known-good immutable set remains usable indefinitely. Scan and publish fail closed when the manifest is missing, malformed, changes during scanning, comes from the future, or the local scan result is not `clean`.
...
- Release assurance is provider-free: the signed empty webhook probe and release report must attest `providerRequests: { deepseek: 0, embedding: 0 }`. It proves gateway/Dapr/bot reachability only, never LINE delivery or reply-token behavior. The normal rollback copies the known-good revision into a new revision; use a manual image change only as bounded emergency fallback.
