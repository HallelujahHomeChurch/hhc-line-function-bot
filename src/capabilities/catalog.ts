...
export const CAPABILITY_CATALOG: FunctionDefinition[] = [
  ...
  },
  {
    ...
    agentCapability: {
      ...
      semanticDescription:
        "使用者明確要求保存、新增、修改或刪除服事安排時，使用本工具準備預覽。支持使用者提供的單筆或多筆安排，不要求完整月份；保留原文於 content，將日期、聚會、人員與相關備註整理成 entries，排除無關公告；依工具列出的 domainKey 選擇服事類型，人員名稱不是類型。伺服器驗證資料並一次預覽全部月份。查詢工具不會保存資料，不必先查詢才能準備新內容。工具僅建立待確認預覽，尚未保存；只有使用者確認後才執行寫入。",
      operations: []
    },
