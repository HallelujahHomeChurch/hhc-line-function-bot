...
import { kernelLocalLiveDirectUserId, type KernelLocalLiveCaseId } from "./contracts.js";
  ...
  Object.freeze([
    journey("schedule-explicit", "execute", [
      textTurn(
        "schedule-explicit",
        0,
        directUser("schedule-explicit"),
        "查 synthetic service 2026-07-27 投影服事"
      )
    ]),
    journey("schedule-refinement", "active_task_continuation", [
      textTurn(
        "schedule-refinement",
        0,
        directUser("schedule-refinement"),
        "查 synthetic service 2026-07-27 服事"
      ),
      textTurn("schedule-refinement", 1, directUser("schedule-refinement"), "只看投影")
    ]),
    journey("schedule-ambiguity", "clarify", [
      textTurn("schedule-ambiguity", 0, directUser("schedule-ambiguity"), "查服事")
    ]),
    journey("capability-switch", "explicit_switch", [
      textTurn(
        "capability-switch",
        0,
        directUser("capability-switch"),
        "查 synthetic service 2026-07-27 投影服事"
      ),
      textTurn(
        "capability-switch",
        1,
        directUser("capability-switch"),
        "改查知識 synthetic alpha procedure"
      )
    ]),
    journey("knowledge-follow-up", "grounded_follow_up", [
      textTurn(
        "knowledge-follow-up",
        0,
        directUser("knowledge-follow-up"),
        "查知識 synthetic alpha procedure"
      ),
      textTurn(
        "knowledge-follow-up",
        1,
    ...
    journey("provider-unavailable", "providers_unavailable", [
      textTurn(
        ...
        0,
        directUser("provider-unavailable"),
        "查 synthetic service 2026-07-27 Projection 服事"
      )
    ]),
    journey("write-preview-confirm", "confirmed_local_write", [
      fileTurn("write-preview-confirm", 0, directUser("write-preview-confirm")),
      textTurn("write-preview-confirm", 1, directUser("write-preview-confirm"), "是"),
      textTurn("write-preview-confirm", 2, directUser("write-preview-confirm"), "一般資料"),
      textTurn(
        "write-preview-confirm",
        3,
        directUser("write-preview-confirm"),
        "Synthetic Document"
      ),
      textTurn("write-preview-confirm", 4, directUser("write-preview-confirm"), "保存")
    ])
  ] satisfies KernelLocalLiveJourneyDefinition[]);

function directUser(caseId: KernelLocalLiveCaseId) {
  return { type: "user" as const, userId: kernelLocalLiveDirectUserId(caseId) };
}
