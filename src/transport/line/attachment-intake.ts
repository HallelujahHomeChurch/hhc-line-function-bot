...
export interface AttachmentIntakeInput {
  ...
  now: Date;
  textHandlers?: readonly TextMessageHandler[];
}

export async function handleAttachmentIntake(
  input: AttachmentIntakeInput
): Promise<FunctionExecutionResult | undefined> {
  if (input.event.type !== "message") return undefined;
  if (input.event.message?.type !== "text") return handleAttachmentMessage(input);
  if (!input.event.message.text) return undefined;

  const profile = await authorizedAttachmentProfile(input);
  if (!profile.enabledFunctions.includes("save_resource")) return undefined;
  const context = {
    profile,
    event: input.event,
  ...
  const request = { text: input.event.message.text };
  for (const handler of input.textHandlers ?? []) {
    if (handler && (await handler.matches(request, context))) {
...
async function authorizedAttachmentProfile(
  input: AttachmentIntakeInput
): Promise<BotProfileConfig> {
  const configured = input.configuredFunctions ?? input.profile.enabledFunctions;
  if (!configured.includes("save_resource")) return input.profile;
  if (
    input.profile.enabledFunctions.includes("save_resource") &&
    !input.profile.permissionRequiredFunctions.includes("save_resource")
  ) {
  ...
  try {
    ...
    // Restricted attachment intake fails closed.
  }
  return allowed.includes("save_resource")
    ? {
        ...input.profile,
        enabledFunctions: [...input.profile.enabledFunctions, "save_resource"]
      }
    : {
        ...input.profile,
        enabledFunctions: input.profile.enabledFunctions.filter((name) => name !== "save_resource")
      };
}
