...
The agent turn runtime centralizes natural-language task execution after LINE entrance checks. It handles pending text sessions, pre-route resource recall, admin natural-language actions, deterministic capability generation, model planning, plan validation, resolver selection, required-slot collection, in-flight duplicate locks, registered function execution, active-task transitions, and sanitized turn traces. Its precedence is pending confirmation/cancellation, resolver selection, required-slot collection, attachment workflow, an explicit function switch, active-task continuation, then a new plan. A bare `保存` therefore confirms the current write instead of starting generic text memory, and a resolver answer resumes the grounded original arguments without asking the LLM to plan again.
