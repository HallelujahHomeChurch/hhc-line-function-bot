...
Pasted schedules can contain several months, emoji, reminders and unrelated announcements. The helper organizes structured `entries` while retaining `content` for comparison. Schedule tool identifiers come from the current profile registry; assignee names do not select the domain. The full preview includes every included month, and one approval publishes those monthly records atomically. Other months remain unchanged. Recoverable preparation results allow another authorized attempt within the existing turn budget. Exact structured draft edits preserve untouched entries; invalid edits revoke the old approval without discarding the draft.
...
Each profile may declare `schedulePolicy.meetingReferences` with central Meeting keys and aliases used by imported roster documents. These references contain no weekdays or start/end times. Fixed meeting timing comes from hhc-web-api `/priv/meeting-occurrences` over the existing Dapr sidecar, with a 3-second request timeout and at most 60 seconds of caching. `下一場` shares this resolver across synchronized, saved, and live roster sources; cancelled or missing referenced occurrences are ineligible, and moved occurrences retain their original roster identity while displaying the resolved date. API failure does not fall back to invented fixed meeting times. Independent saved schedules that are not mapped to a church Meeting retain their explicit document dates. Personnel assignments and `schedulePolicy.domains` remain in the roster domain. Deploy hhc-web-api contract `7f44c682` or newer before this consumer; migrate any external profile files from `meetingWindows` to `meetingReferences` by retaining only keys and aliases.
...
Weekly Paper downloads in the main profile honor the Website API public-access switch. When the API returns `bulletin_disabled`, the bot rechecks the linked Account’s current LINE function permission. An active authorized account receives only the fixed website entry, where website login and current bulletin access policy are checked again. Unbound users are directed to the existing `登入` binding flow; inactive or unauthorized accounts receive an access-denied explanation. Account failures return an unavailable response without a button. No private PDF URL or bulletin content is sent through LINE. Previously delivered PDF links are denied by Asset API after grant revocation completes; downloaded or cached copies cannot be recalled.
