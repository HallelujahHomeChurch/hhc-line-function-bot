...
Bot deployment is manifest-driven: `aca.containerapp.yaml` owns the bot's Dapr, ingress, probe, scale, resource, mount, and environment-variable structure, while `scripts/deploy-aca.sh` owns environment values, secret-reference names and values, image selection, rendering, apply order, and rollout verification. The script deploys internal SearXNG first, renders and applies the bot manifest, restores the required Dapr configuration, waits for the new bot revision to be healthy, and then updates the signature-refresh, attachment-scan, and catalog-sync jobs. It binds the same Azure Files share read/write for refresh and read-only for scanning, provisions the storage key and scan queue connection directly from their Azure resources, and copies only the exact preconfigured ACA secrets needed by each job.
